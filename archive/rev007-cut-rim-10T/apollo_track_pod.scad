...
//
//  REV 006b DELTAS (2026-07-25, idler rides IN the lug gap — Rev 004 returns):
...
sprocket_teeth = 10;   // Rev 006: the cut rim (20.5 wide) is narrower than the
                       // 22 mm lug gap, so the whole drum passes between the
                       // lug pairs — lugs never meet the rim and the vertical
                       // lug face stops mattering. 10T -> cord Ø191 (kit
                       // circle), wheel Ø179, +10% drive force vs the 11T.
                       // (Rev 005c was 11T on the uncut Ø165x35 rim.)
...
a_frac     = 0.433; // shock bolt station as fraction of C (Rev 006b: with the
...
drop   = 38;                          // pivot sits 38 above idler axle line
                                      // (Rev 006b: BACK to the Rev 004 value.
...
y_keel = -(sprocket_od/2 + rib_h + 10); // Rev 006b: back in the Rev 004 window
...
module idler_wheel(){
  // Rev 006b: stepped profile — the Ø108 tread band (~20 wide) runs IN the
  ...
  color([0.16,0.16,0.17]) difference(){
    union(){
      cylinder(h=20, d=D, center=true);          // tread band, in the lug gap
...
module sprocket(){
  ...
  color([0.55,0.55,0.58]){                     // cut rim ring + spokes
    difference(){ cylinder(h=sprocket_w, d=sprocket_od, center=true);
                  cylinder(h=sprocket_w+2, d=sprocket_od-2*rim_wall, center=true); }
    for (i=[0:5]) rotate([0,0,i*60])           // spoke contact points (visual)
      translate([(130/2 + sprocket_od/2-rim_wall)/2, 0, 0])
        cube([(sprocket_od/2-rim_wall) - 130/2 + 2, 12, sprocket_w], center=true);
  }
  color([0.55,0.62,0.52]){                     // printed ABS (sage)
    difference(){                              // clamp-shell skin on the drum
      cylinder(h=sprocket_w, d=sprocket_od + 2*shell_t, center=true);
      cylinder(h=sprocket_w+2, d=sprocket_od - 0.5, center=true);
    }
...
module arm3d(zi, slot, ang, szs){
  translate([pivot[0], pivot[1], 0]) rotate([0,0,-na+ang]){
    ...
    // (Rev 006b: brace restored to the Rev 004 position 28..58 — the full
